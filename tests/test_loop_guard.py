...
def test_loop_guard_preserves_completed_mutation_but_allows_verification() -> None:
    guard = ToolLoopGuard()

    assert guard.record("file_mutation", {"operation": "edit", "path": "test.txt"}) == 1
    assert guard.record("read_file", {"path": "test.txt"}) == 1
